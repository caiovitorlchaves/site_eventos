...
def subscribers_by_link(link, event_id):
    subs_repo = SubscribersRepository()
    subs_manage = SubscriberManager(subs_repo)
    
    http_request = HttpRequest(param={"link": link, "event_id": event_id})

    http_response = subs_manage.get_subscribers_by_link(http_request)
...
def link_ranking(event_id):
    ...
    http_response = subs_manage.get_event_ranking(http_request)
